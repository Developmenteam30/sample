...
$sql = "SELECT * ,(SELECT SUM(due_amt) FROM `transaction` WHERE `entry_type` = 'create sales invoice' AND `name` = customer.company_name AND `date` <= '$date_to' AND `company_email` = '$company_email') AS `values` FROM customer WHERE customer.company_email = '$company_email' AND customer.id = '$name_id'";
$new = $conn->query($sql);
$row = $new->fetch_assoc();

//$date_curent  = date('Y-m-d', strtotime($dates. ' + 2 days'));
$dates = $date_to; 
$datei = $date_to; 
